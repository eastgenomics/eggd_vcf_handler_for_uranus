...
mark-section "filtering variants"
# retain variants that are within ROIs (bed file), have at least one allele >0.03 AF, have DP >99
bedtools intersect -header -a $vcf_path -b $bed_path | bcftools view -i "FORMAT/AF[*]>0.03" - | bcftools view -i "DP>99" - -o ~/$vcf_prefix.filter.vcf

mark-section "annotating variants"
